...
let REPLY_POST_IMAGE_SIZE:CGFloat  = 80
let REPLY_POST_IMAGE_SPACE:CGFloat = 10

class ReplyPostVC:UIViewController {
    
    static let DID_REPLY_POST_NOTIFICATION = "DID_REPLY_POST_NOTIFICATION "
    ...
    func send(sender:AnyObject?) {
        ...
        if let t = token, pid = postID {
            request(.POST, COMMENT_POST, parameters: ["token":t, "body":textView.text, "postid":pid], encoding: .JSON).responseJSON(completionHandler: { [weak self](response) -> Void in
                debugPrint(response)
                if let StrongSelf = self {
                    if let d = response.result.value {
                        ...
                        if json["state"] == "successful" {
                            ...
                            if (StrongSelf.images.count > 0 && commentid != .null) {
                                for k in 1...StrongSelf.images.count {
                                    upload(.POST, UPLOAD_AVATAR_URL, multipartFormData: { multipartFormData in
                                        let dd = "{\"token\":\"\(t)\", \"type\":\"\(-7)\", \"commentid\":\"\(commentid.stringValue)\", \"number\":\"\(k)\"}"
                                        print(dd)
                            ...
                            NSNotificationCenter.defaultCenter().postNotificationName(ReplyPostVC.DID_REPLY_POST_NOTIFICATION, object: nil)
...
extension ReplyPostVC:UIScrollViewDelegate {
...
extension ReplyPostVC:UITextViewDelegate {
...
extension ReplyPostVC:UICollectionViewDataSource {
...
extension ReplyPostVC:UICollectionViewDelegate {
...
extension ReplyPostVC:UICollectionViewDelegateFlowLayout {
    func collectionView(collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAtIndexPath indexPath: NSIndexPath) -> CGSize {
        return CGSize(width: REPLY_POST_IMAGE_SIZE, height: REPLY_POST_IMAGE_SIZE)
    }
    
    func collectionView(collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, insetForSectionAtIndex section: Int) -> UIEdgeInsets {
        return UIEdgeInsetsMake(REPLY_POST_IMAGE_SPACE, REPLY_POST_IMAGE_SPACE,REPLY_POST_IMAGE_SPACE,REPLY_POST_IMAGE_SPACE)
    }
}

extension ReplyPostVC:UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension ReplyPostVC:GMImagePickerControllerDelegate {
...
extension ReplyPostVC:ImageCollectionViewCellDelegate {
